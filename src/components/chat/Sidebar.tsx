...
export function Sidebar() {
  ...
  const displayUsers = memberSearchQuery
    ? (searchResults as any)?.data || []
    : defaultUsers;
  const shouldShowUsers =
    showMemberSearch && (memberSearchQuery || defaultUsers.length > 0);
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            {contact.unreadCount && (
              <Badge className="neo-gradient text-white text-xs">
                {contact.unreadCount}
              </Badge>
            )}
          </div>
    ...
    >
      <div className="flex items-center space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground truncate">
            {group.lastMessage || "No messages yet"} • {group.memberCount}{" "}
            members
          </p>
        </div>
      </div>
              ...
              {shouldShowUsers && (
                <div className="border rounded-lg max-h-48 overflow-hidden flex flex-col">
                  <div className="p-2 bg-accent/10 border-b text-sm font-medium">
                    {memberSearchQuery
                      ? `Search results for "${memberSearchQuery}"`
                      : "Suggested members"}
                  </div>
